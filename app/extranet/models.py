...
class LeaveRequest(models.Model):
    ...
    rh_validated = models.BooleanField(
    ...
    class Meta:
        ordering = [
            "-submitted_at"
        ]  # Trie par date de soumission décroissante
...
class UserProfile(models.Model):
    ...
    site = models.CharField(
        max_length=10, choices=SITE_CHOICES, default="tunisie"
    )
...
def get_leave_balance_detailed(user):
    ...
    def calculate_leave_days(leave):
        ...
        if site == "france":
            ...
            for leave_date in leave_dates:
                ...
                if (leave_date.weekday() == 4 and  # vendredi
                    leave_date + timedelta(days=1) not in leave_dates):
                    total_days += 1  # ajouter le samedi
                # Si c'est un jeudi et que le vendredi est férié et samedi pas dans congé
                elif (leave_date.weekday() == 3 and  # jeudi
                      leave_date + timedelta(days=1) in holidays and  # vendredi férié
                      leave_date + timedelta(days=2) not in leave_dates):  # samedi pas en congé
                    total_days += 1  # ajouter le samedi
    ...
    if hasattr(user, "profile") and user.profile.carry_over > 0:
        ...
        if prev_work_start <= prev_work_end:
            ...
            prev_days_taken = sum([calculate_leave_days(leave) for leave in prev_leaves])
...
class UserLeaveBalanceCache(models.Model):
    ...
    """
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leave_balance_cache')
    year = models.IntegerField()
    acquired_days = models.DecimalField(max_digits=4, decimal_places=1, default=0, help_text="Jours acquis dans l'année")
    taken_days = models.DecimalField(max_digits=4, decimal_places=1, default=0, help_text="Jours pris dans l'année")
    carry_over_days = models.DecimalField(max_digits=4, decimal_places=1, default=0, help_text="Report de l'année précédente")
    remaining_days = models.DecimalField(max_digits=4, decimal_places=1, default=0, help_text="Solde restant")
...
class UserMonthlyReportCache(models.Model):
    ...
    """
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='monthly_report_cache')
    year = models.IntegerField()
    month = models.IntegerField()
    days_at_office = models.IntegerField(default=0, help_text="Jours travaillés au bureau")
    days_telework = models.IntegerField(default=0, help_text="Jours en télétravail")
    days_leave = models.DecimalField(max_digits=4, decimal_places=1, default=0, help_text="Jours de congés pris")
    total_workdays = models.IntegerField(default=0, help_text="Total jours ouvrés du mois")
    last_updated = models.DateTimeField(auto_now=True)
