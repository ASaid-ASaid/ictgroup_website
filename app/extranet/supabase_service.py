...
try:
    ...
    logging.warning("Supabase n'est pas configuré. Les fonctionnalités Supabase sont désactivées.")
...
class SupabaseService:
    ...
    def log_user_activity(self, user_id: int, action: str, details: Dict[str, Any] = None) -> bool:
    ...
    def get_user_activity(self, user_id: int, limit: int = 50) -> List[Dict]:
        ...
        try:
            result = self.client.table('user_activity_logs') \
                .select('*') \
                .eq('user_id', user_id) \
    ...
    def upload_document(self, bucket: str, file_path: str, file_data: bytes) -> Optional[str]:
    ...
    def get_document_url(self, bucket: str, file_path: str) -> Optional[str]:
        ...
        try:
            result = self.client.storage.from_(bucket).create_signed_url(file_path, 3600)  # 1 heure
    ...
    def get_dashboard_stats(self, user_id: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_id:
                activity_count = self.client.table('user_activity_logs') \
                    .select('*', count='exact') \
                    .eq('user_id', user_id) \
                    .execute()
