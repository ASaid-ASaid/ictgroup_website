...
from ..models import UserProfile, LeaveRequest, TeleworkRequest, get_leave_balance
...
def _handle_user_creation(request):
    ...
    try:
        ...
        if form.is_valid():
            ...
            logger.info(f"[user_admin] Utilisateur créé: {user.username} par {request.user.username}")
...
def _handle_user_update(request):
    ...
    try:
        ...
        logger.info(f"[user_admin] Utilisateur mis à jour: {user.username} par {request.user.username}")
...
def _handle_user_deletion(request):
    ...
    try:
        ...
        logger.info(f"[user_admin] Utilisateur supprimé: {username} par {request.user.username}")
...
def _export_users_csv():
    ...
    writer.writerow(['Login', 'Nom', 'Prénom', 'Email', 'Rôle', 'Site', 'Manager', 'RH'])
...
def validation(request):
    ...
    if hasattr(user, "profile"):
        ...
        if can_validate_as_manager and can_validate_as_rh:
            ...
            leaves = LeaveRequest.objects.filter(
                Q(status="pending", user__profile__manager=user, manager_validated=False) |
                Q(status="pending", user__profile__rh=user, manager_validated=True, rh_validated=False)
            ).distinct()
            ...
            # RH uniquement
            leaves = LeaveRequest.objects.filter(
                status="pending", user__profile__rh=user, manager_validated=True, rh_validated=False
            )
    ...
    if request.method == "POST":
        if "leave_id" in request.POST:
            ...
            # Validation manager
            if (action == "manager_approve" and 
                can_validate_as_manager and 
                leave.user.profile.manager == user and
                not leave.manager_validated):
                leave.manager_validated = True
                leave.save()
                messages.success(request, "Demande validée par le manager.")
            
            # Validation RH
            elif (action == "rh_approve" and 
                  can_validate_as_rh and 
                  leave.user.profile.rh == user and
                  leave.manager_validated and 
                  not leave.rh_validated):
            ...
            if (action == "manager_approve" and 
                can_validate_as_manager and 
                tw.user.profile.manager == user and
                not tw.manager_validated):
    ...
    return render(
        ...
        "extranet/validation.html",
        {
            ...
            "can_validate_as_manager": can_validate_as_manager if hasattr(user, 'profile') else False,
            "can_validate_as_rh": can_validate_as_rh if hasattr(user, 'profile') else False,
        },
    )
