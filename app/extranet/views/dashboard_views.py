...
def home(request):
    ...
    recent_teleworks = TeleworkRequest.objects.filter(user=user).order_by('-submitted_at')[:5]
...
def _get_user_statistics(user):
    ...
    return {
        ...
        'total_requests_this_year': leaves_this_year.count() + telework_this_year.count(),
    }
