...
class DatabasePerformanceTests(PerformanceMonitor):
    ...
    def test_query_performance(self):
        ...
        if error:
            ...
            self.log_result(
                "DB: UserProfile JOIN", duration, "SUCCESS", {"count": len(result)}
            )
        ...
        if error:
            self.log_result(
                "DB: LeaveRequest Query", duration, "ERROR", {"error": error}
            )
        else:
            self.log_result(
                "DB: LeaveRequest Query", duration, "SUCCESS", {"count": len(result)}
            )
...
class WebPerformanceTests(PerformanceMonitor):
    ...
    def test_single_page(self, url, description):
        ...
        if DJANGO_AVAILABLE and self.client:
            ...
            if error:
                self.log_result(
                    f"Web: {description}", duration, "ERROR", {"error": error}
                )
            else:
            ...
            try:
                ...
                duration = time.time() - start_time
                self.log_result(
                    f"HTTP: {description}", duration, "ERROR", {"error": str(e)}
                )
...
class PerformanceReporter:
    ...
    def generate_summary(self):
        ...
        slow_tests = sorted(self.results, key=lambda x: x["duration_ms"], reverse=True)[
            :5
        ]
        print(f"\n🐌 Tests les plus lents:")
